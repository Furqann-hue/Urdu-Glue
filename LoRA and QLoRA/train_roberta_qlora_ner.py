"""
Fine-tune RoBERTa-XL model with QLoRA (Quantized LoRA) technique on Urdu NER (Named Entity Recognition) dataset
QLoRA uses 4-bit quantization for memory-efficient fine-tuning
Token Classification task - Assign NER tag to each token
"""
import os
import tarfile
import torch
...
class NERDataset(Dataset):
    """Dataset class for NER (Named Entity Recognition) task"""
...
def load_ner_data(file_path: str) -> List[Dict]:
    ...
    Format: word\tNER_tag (one per line, blank lines separate sentences)
    
    Args:
        file_path: Path to the data file (can be .tar.gz, .tar.xz, or .txt)
...
def create_label_mapping(data: List[Dict]) -> Tuple[Dict[str, int], Dict[int, str]]:
    ...
    # Sort labels for consistent mapping, but ensure "O" (or "Other") comes first if present
    sorted_labels = sorted(all_labels)
    if "O" in sorted_labels:
        sorted_labels.remove("O")
        sorted_labels.insert(0, "O")
    elif "Other" in sorted_labels:
        sorted_labels.remove("Other")
        sorted_labels.insert(0, "Other")
    
    # Create mappings
    label_to_id = {label: idx for idx, label in enumerate(sorted_labels)}
    id_to_label = {idx: label for label, idx in label_to_id.items()}
    
    print(f"✓ Found {len(label_to_id)} unique NER tags")
    print(f"  Tags: {sorted_labels[:20]}{'...' if len(sorted_labels) > 20 else ''}")
    
    return label_to_id, id_to_label


def load_ner_dataset(train_split: float = 0.8, random_state: int = 42) -> Dict:
    """
    Load NER dataset from all available files and create train/validation splits
    ...
    print(f"Data directory: {NER_DIR}")
    
    # Find all data files
    data_files = []
    for ext in ['.tar.gz', '.tar.xz', '.txt']:
        for file in os.listdir(NER_DIR):
            if file.endswith(ext):
                data_files.append(os.path.join(NER_DIR, file))
    
    if not data_files:
        raise FileNotFoundError(f"No NER data files found in {NER_DIR}")
    ...
    for file_path in data_files:
        try:
            file_data = load_ner_data(file_path)
            all_data.extend(file_data)
            print(f"  ✓ Loaded {len(file_data)} sentences from {os.path.basename(file_path)}")
        except Exception as e:
            print(f"  ⚠️  Warning: Could not load {os.path.basename(file_path)}: {e}")
    
    if not all_data:
        raise ValueError("No data could be loaded from any file!")
    
    print(f"\nTotal: {len(all_data)} sentences from all files")
...
def main():
    ...
    print("=" * 60)
    print("RoBERTa-XL QLoRA Fine-tuning for Urdu NER (Named Entity Recognition)")
    print("=" * 60)
